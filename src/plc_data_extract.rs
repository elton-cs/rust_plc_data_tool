...
pub fn extract_plc_data() -> Vec<TestSet> {
    ...
    let end_string = "]";
    let messages = json_to_vector(file_path, start_string, end_string);

    let start_string = "\"payload\"";
    let new_vec = string_search_in_vector(messages, start_string);
    // println!("{:#?}", new_vec);

    let new_vec = format_data(&new_vec);
    // println!("{:#?}", new_vec);

    let new_vec = remove_odd_indices(new_vec);
    // println!("{:#?}", new_vec);

    // Working data extraction procedure:
    // let file_path = "test_data_final.json";
    let big_data = split_data_vector(new_vec, 3);

    let all_test_sets_vector: Vec<TestSet> = big_data.iter().map(|single_test_set| {
        ...
        let title: TestType = get_test_type(&single_test_set[0]);

        // splitting the connector type from the result of each test
        let connector_and_result = split_connector_from_result(&single_test_set[2]);

        // connector type of each test
        let connectors_vec: Vec<ConnectorType> = connector_and_result
        .iter()
        .map(|value| get_connector_type(value))
        .collect();

        // measured values of each test (voltage, current, ohms etc...)
        let value_vec = split_connector_from_result(&single_test_set[1]);
        let value_vec: Vec<f32> = value_vec
        .iter()
        .map(|value| get_test_value(value))
        .collect();

        // results of each test (PASS/FAIL)
        let results_vec: Vec<TestResult> = connector_and_result
        .iter()
        .map(|value| get_test_result(value))
        .collect();
